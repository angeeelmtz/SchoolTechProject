...
if (isset($_POST['school_name']) &&
    ...
    if (empty($school_name)) {
        $em  = "Se requiere el nombre de la escuela";
        header("Location: ../settings.php?error=$em");
        exit;
    }else if (empty($slogan)) {
        $em  = "El nombre del slogan es obligatorio";
        header("Location: ../settings.php?error=$em");
        exit;
    }else if (empty($about)) {
        $em  = "La descripción es obligatorio";
        header("Location: ../settings.php?error=$em");
        exit;
    }else if (empty($current_year)) {
        $em  = "Se requiere año actual";
        header("Location: ../settings.php?error=$em");
        exit;
    }else if (empty($current_semester)) {
        $em  = "El nombre del semestre actual es obligatorio";
        header("Location: ../settings.php?error=$em");
        exit;
    }else {
        $id = 1;
        ...
        $sm = "Configuración actualizada correctamente";
